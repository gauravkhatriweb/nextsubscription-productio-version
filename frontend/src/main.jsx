...
const initializeTheme = async () => {
  ...
  // Load custom theme from backend settings
  try {
    const apiBase = (import.meta.env.VITE_API_BASE_URL && String(import.meta.env.VITE_API_BASE_URL).trim().replace(/\/+$/, '')) || 'http://localhost:3000'
    const response = await axios.get(`${apiBase}/api/admin/settings`)
    
    if (response.data.success && response.data.settings.theme) {
      ...
      if (response.data.settings.siteName) {
        document.title = response.data.settings.siteName
      }
    }
  } catch (error) {
