 ...
 * Next Subscription platform. It handles user authentication,
 ...
 * authentication status, and session persistence for users.
 * 
...
export const UserProvider = ({ children }) => {
  ...
  const [user, setUser] = useState(null) // Current user object (user)
  ...
  useEffect(() => {
    try {
      const rawLocal = localStorage.getItem('nextsubscription_auth')
      const rawSession = sessionStorage.getItem('nextsubscription_auth')
   ...
   * User Login Function
   * 
   * Authenticates a user and manages session persistence based on
  ...
  const loginUser = async (email, password, remember) => {
    ...
    try {
      ...
      const normalized = { type: 'user', ...(data?.user || {}) }
      setUser(normalized)
      setIsAuthenticated(true)
      
      // Persist session based on remember preference
      try {
        const payload = JSON.stringify({ type: 'user', user: normalized, token: data?.token || null })
        if (remember) {
          localStorage.setItem('nextsubscription_auth', payload)
          sessionStorage.removeItem('nextsubscription_auth')
        } else {
          sessionStorage.setItem('nextsubscription_auth', payload)
          localStorage.removeItem('nextsubscription_auth')
        }
      } catch (_) {
   ...
   * and clearing all stored session data. Handles user
  ...
  const updateUserProfile = (updatedData) => {
    ...
    try {
      ...
      if (raw) {
        ...
        if (rawLocal) {
          localStorage.setItem('nextsubscription_auth', newPayload)
        } else if (rawSession) {
          sessionStorage.setItem('nextsubscription_auth', newPayload)
        }
      }
    } catch (_) {
