 ...
 * 
 * Profile management page for authenticated users. Displays account information,
 * allows profile picture upload/update/delete, and provides account actions.
 * 
 * Key Features:
 * - Profile picture display with upload/update/delete functionality
 * - Account information display (name, email, verification status)
 * - Account details (ID, member since, last updated)
 * - Profile picture upload modal with preview
 * - Logout functionality
 * - Form validation for file uploads (JPG, PNG, max 2MB)
 * 
 * Authentication:
 ...
 * - POST /api/users/upload-profile-pic - Upload new profile picture
 ...
 * 
 * Main profile management component with account information and profile picture controls.
 * 
...
const UserProfile = () => {
   ...
   * 
   * Returns the API base URL from environment variable or defaults to localhost:3000
   * Used for constructing profile picture URLs and API endpoints
   */
  const getApiBaseUrl = () => {
  ...
  useEffect(() => {
    const fetchProfile = async () => {
      // Wait for auth initialization and ensure user is authenticated user
  ...
  const handleUploadProfilePicture = async () => {
    ...
    try {
      ...
      const response = await axios.post(
        `${apiBase}/api/users/upload-profile-pic`,
        formData,
        {
          headers: {
  ...
  const handleUpdateProfilePicture = async () => {
    ...
    try {
      ...
      const response = await axios.put(
        `${apiBase}/api/users/update-profile-pic`,
        formData,
        {
          headers: {
            'Authorization': `Bearer ${token}`,
  ...
  const handleDeleteProfilePicture = async () => {
    ...
    try {
      ...
      const response = await axios.delete(
        `${apiBase}/api/users/delete-profile-pic`,
        {
          headers: {
            'Authorization': `Bearer ${token}`
          }
              ...
              <div className='flex flex-col sm:flex-row items-center gap-6'>
                ...
                <div className='relative group'>
                  ...
                  <div className='relative w-24 h-24 rounded-full overflow-hidden bg-gradient-to-br from-brand-primary to-brand-secondary flex items-center justify-center text-3xl font-bold text-theme-primary shadow-xl border-4 border-white/20 group-hover:border-white/30 transition-all duration-300'>
                    {profile?.profilePic ? (
                      <img 
                      src={`${getApiBaseUrl()}${profile.profilePic}`} 
                        alt='Profile' 
