/**
 * userVerifyOtp.jsx - OTP Verification Page
 * 
 * Page for users to verify their account using a 6-digit OTP sent via email.
 ...
 * 3. POST request to /api/users/verify-otp
 * 4. On success, account is verified and user redirected
 * 5. On error or expiry, user can request new OTP
 * 
 * API Endpoints:
 * - POST /api/users/verify-otp - Verify OTP
 * - POST /api/users/send-verification-otp - Resend OTP
 * 
...
/**
 * UserVerifyOtp Component
 * 
 * Main OTP verification form with individual digit inputs and timer management.
 * 
...
const UserVerifyOtp = () => {
   ...
   * Timer States
   * 
   * - timeLeft: Main countdown timer (10 minutes = 600 seconds)
   * - canResend: Whether user can request new OTP
   * - resendCooldown: Cooldown timer for resend (1 minute = 60 seconds)
   */
   ...
   * 
   * Decrements timeLeft every second. When timer reaches 0,
   * shows error message and expires the OTP.
   */
  useEffect(() => {
    if (timeLeft > 0) {
   ...
   * Resets timer, clears existing OTP, and applies cooldown.
   * 
   * API Endpoint: POST /api/users/send-verification-otp
   */
  const handleResendOtp = async () => {
    ...
    try {
      ...
      await axios.post(`${apiBase}/api/users/send-verification-otp`, {}, {
   ...
   * 
   * API Endpoint: POST /api/users/verify-otp
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const apiBase = (import.meta.env.VITE_API_BASE_URL && String(import.meta.env.VITE_API_BASE_URL).trim().replace(/\/+$/, '')) || 'http://localhost:3000'
      await axios.post(`${apiBase}/api/users/verify-otp`, { otp: otpString }, {
        withCredentials: true,
        headers: {
          'Content-Type': 'application/json',
          'Accept': 'application/json',
