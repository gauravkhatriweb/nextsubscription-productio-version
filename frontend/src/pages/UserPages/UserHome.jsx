...
const UserHome = () => {
  ...
  const [showPremiumPlansBanner, setShowPremiumPlansBanner] = useState(true)
  const [currentPaymentMethod, setCurrentPaymentMethod] = useState('card')
  
  // Mock data
  const savedServices = [
    { id: 1, name: 'Netflix', category: 'Entertainment', icon: '🎬' },
    { id: 2, name: 'Spotify', category: 'Music', icon: '🎵' },
    { id: 3, name: 'Adobe Creative Cloud', category: 'Productivity', icon: '🎨' }
  ]
  
  const recentSubscriptions = [
    { id: 1, service: 'Netflix', date: '2 hours ago', amount: 15.99 },
    { id: 2, service: 'Spotify', date: 'Yesterday', amount: 9.99 }
  ]
  
  const paymentMethods = [
    { id: 'card', name: 'Credit Card', icon: '💳', description: 'Visa ending in 4242' },
    { id: 'paypal', name: 'PayPal', icon: '🔵', description: 'Connected account' },
    { id: 'bank', name: 'Bank Transfer', icon: '🏦', description: 'Direct debit' }
  ]
  ...
  useEffect(() => {
    const fetchProfile = async () => {
      if (!isInitialized || !isAuthenticated || !user || user.type !== 'user') return
      
      try {
        setLoading(true)
        setProfileError(null)
        
        const raw = localStorage.getItem('nextsubscription_auth') || sessionStorage.getItem('nextsubscription_auth')
        let token = null
        if (raw) {
          try {
  ...
  const handleQuickAction = (action) => {
    switch (action) {
      case 'schedule':
        toast.info('Schedule payment feature coming soon!')
        break
      case 'notifications':
        toast.info('Notifications feature coming soon!')
        break
      case 'saved-services':
        toast.info('Saved services management coming soon!')
        break
      case 'wallet':
    ...
    <div className="min-h-screen bg-theme-base text-theme-primary relative overflow-hidden">
      ...
      <main className="max-w-6xl mx-auto px-4 sm:px-6 py-6 relative z-10">
        ...
        >
          ...
          <p className="text-theme-secondary" style={{ fontFamily: 'Inter, sans-serif' }}>Your subscriptions, simplified and secure.</p>
        </motion.div>
                ...
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-4 flex-1">
                    ...
                    <div className="flex-1">
                      <h3 className="text-lg font-bold text-theme-primary mb-1" style={{ fontFamily: 'Poppins, sans-serif' }}>Welcome bonus: 20% off your first 3 subscriptions!</h3>
                      <p className="text-sm text-theme-secondary" style={{ fontFamily: 'Inter, sans-serif' }}>Valid until end of month • No minimum payment required</p>
                    </div>
                  </div>
                  <button
        ...
        >
          <motion.button
            ...
            className="w-full p-8 bg-gradient-to-r from-brand-primary via-brand-secondary to-brand-accent rounded-3xl text-theme-primary shadow-[0_8px_30px_rgba(0,0,0,0.25)] hover:shadow-[0_12px_40px_rgba(0,0,0,0.35)] transition-all duration-300"
            style={{ fontFamily: 'Poppins, sans-serif' }}
            aria-label="Manage subscriptions - Navigate to management page"
            role="button"
          >
            <div className="text-center">
              <div className="text-6xl mb-4">📋</div>
              <h2 className="text-3xl font-bold mb-2">Manage Subscriptions</h2>
              <p className="text-lg opacity-90">Handle all your subscriptions in one place</p>
            </div>
          </motion.button>
        </motion.div>
        
        {/* Premium Plans Banner */}
        {showPremiumPlansBanner && (
          <motion.div
            ...
            className="mb-6"
          >
            <div className="bg-gradient-to-r from-brand-secondary to-brand-accent rounded-2xl p-6 text-theme-primary shadow-[0_8px_30px_rgba(0,0,0,0.25)]" style={{ fontFamily: 'Poppins, sans-serif' }}>
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-4">
                  <span className="text-3xl">⭐</span>
                  <div>
                    <h3 className="text-lg font-bold mb-1">Premium Plans</h3>
                    <p className="text-sm opacity-90">Enhanced subscription features for your comfort & value</p>
                  </div>
                </div>
                <button
                  onClick={() => setShowPremiumPlansBanner(false)}
        ...
        >
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-4">
            {[
              { id: 'schedule', icon: '📅', label: 'Schedule Payment' },
              { id: 'notifications', icon: '🔔', label: 'Notifications' },
              { id: 'saved-services', icon: '💾', label: 'Saved Services' },
        ...
        {/* Language Toggle & Recent Subscriptions */}
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <motion.div
            ...
            className="bg-white/5 backdrop-blur-md glass-border rounded-2xl p-6 shadow-[0_8px_30px_rgba(0,0,0,0.25)]"
          >
            <h4 className="text-lg font-semibold text-theme-primary mb-4" style={{ fontFamily: 'Poppins, sans-serif' }}>Recent Subscriptions</h4>
            {recentSubscriptions.length > 0 ? (
              <div className="space-y-3">
                {recentSubscriptions.map((subscription) => (
                  <div key={subscription.id} className="flex items-center justify-between p-3 bg-white/5 backdrop-blur-md glass-border rounded-xl hover:bg-white/10 transition-all duration-200">
                    <div className="flex-1">
                      <p className="font-medium text-theme-primary text-sm" style={{ fontFamily: 'Inter, sans-serif' }}>{subscription.service}</p>
                      <p className="text-xs text-theme-secondary" style={{ fontFamily: 'Inter, sans-serif' }}>{subscription.date}</p>
                    </div>
                    <p className="font-semibold text-theme-primary text-sm" style={{ fontFamily: 'Inter, sans-serif' }}>{formatCurrencyDisplay(subscription.amount)}</p>
                  </div>
                ))}
              </div>
            ) : (
              <div className="text-center py-8">
                ...
                <p className="text-theme-secondary" style={{ fontFamily: 'Inter, sans-serif' }}>No recent subscriptions</p>
                <p className="text-sm text-theme-subtle" style={{ fontFamily: 'Inter, sans-serif' }}>Your subscription history will appear here</p>
              </div>
            )}
          </motion.div>
        </div>
